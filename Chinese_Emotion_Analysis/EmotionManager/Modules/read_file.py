...
class read_file_part:
    ...
    ntusdposemotion = []    ##financial正向情感词
    ntusdnegemotion = []    ##financial负向情感词
    ...
    def _read_dllg_emotion_file(self):
        ...
        dir_path = re.sub('\\\\read_file\.py', '', path)
        # 打开文件
        bk = xlrd.open_workbook( dir_path+'./res/dic/dalianligong/SenDic.xlsx')
        # 打开工作表
    ...
    def __init__(self):
        path = os.path.abspath(__file__)
        dir_path = re.sub('\\\\read_file\.py', '', path)
        self.extreme = self._read_file_(dir_path+'/res/dic/zhiwang/extent_Lv_6.txt')
        self.very = self._read_file_( dir_path+'/res/dic/zhiwang/extent_Lv_4.txt')
        self.more = self._read_file_( dir_path+'/res/dic/zhiwang/extent_Lv_3.txt')
        self._ish = self._read_file_( dir_path+'/res/dic/zhiwang/extent_Lv_2.txt')
        self.insufficiently = self._read_file_( dir_path+'/res/dic/zhiwang/extent_Lv_1.txt')
        self.over = self._read_file_( dir_path+'/res/dic/zhiwang/extent_Lv_5.txt')
        self.noword = self._read_file_(dir_path+'/res/dic/zhiwang/reversed.txt')
