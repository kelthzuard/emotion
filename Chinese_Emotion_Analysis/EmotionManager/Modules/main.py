...
import re
import jieba.posseg
import EmotionManager.Modules.load_dic
import os
import os.path
import sqlite3
from EmotionManager.Modules.load_dic import load_multi_meaning_word_dic
from functools import reduce
from EmotionManager.Modules.use_dalianligong_dic import _text_processing_
from EmotionManager.Modules.extract_word_pair import getCommentPair
#DATA SEGMENT
__choose__ = 0
...
ntusd_pos_dic = {}                             #financial正面情感词词典
ntusd_neg_dic = {}                             #financial负面情感词词典
...
dic_root_path = os.getcwd()       #情感词典的根目录

neg_corpus_root_path = os.getcwd() + '/EmotionManager/Modules/res/corpus/hotel/neg/'  #酒店评价负面情绪语料库目录
pos_corpus_root_path = os.getcwd() + '/EmotionManager/Modules/res/corpus/hotel/pos/' #酒店评价正面情绪语料库目录
sense_word_kind_set = {}                #具有情感的词性集合

currentDir = re.sub('\\\\main\.py', '', os.path.abspath(__file__))

conn = sqlite3.connect(':memory:')
conn.execute('create table polysemy(a,n,s)')
path = currentDir+'/res/dic/create_by_huzehao/'
...
def __init__no_word_list():
    with open(currentDir+"/res/dic/zhiwang/reversed.txt", encoding="utf-8") as f:
        for items in f:
             item = items.strip()
...
def __init_zhiwang_dic__():
    __path__ = currentDir+"/res/dic/zhiwang/"
    global zhiwang_neg_sen_dic
    global zhiwang_pos_sen_dic
    zhiwang_pos_sen_dic = EmotionManager.Modules.load_dic.load_dic(__path__, "pos_comment.txt", 1)  # 初始化正面情感词词典
    EmotionManager.Modules.load_dic.append_dic(zhiwang_pos_sen_dic, __path__, "pos_sentiment.txt", 1)
    zhiwang_neg_sen_dic = EmotionManager.Modules.load_dic.load_dic(__path__, "neg_sentiment.txt", -1)  # 初始化负面情感词词典
    EmotionManager.Modules.load_dic.append_dic(zhiwang_neg_sen_dic, __path__, "neg_comment.txt", -1)
...
def __init_tsinghua_dic__():
    ...
    __path__ = currentDir+"/res/dic/TS_lijianjun\\"
    tsinghua_pos_dic = EmotionManager.Modules.load_dic.load_dic(__path__, "tsinghua.positive.gb.txt", 1)
    tsinghua_neg_dic = EmotionManager.Modules.load_dic.load_dic(__path__, "tsinghua.negative.gb.txt", -1)

#---------------------------------------
#初始化 financial情感词词典
#---------------------------------------
def __init_ntusd_dic__():
	__path__ = currentDir+"/res/dic/financial/"
	global ntusd_neg_dic
	global ntusd_pos_dic
	ntusd_pos_dic = EmotionManager.Modules.load_dic.load_dic(__path__, "ntusd-positive.txt", 1)
	ntusd_neg_dic = EmotionManager.Modules.load_dic.load_dic(__path__, "ntusd-negative.txt", 1)

#---------------------------------------
#初始化大连理工情感词词典
#---------------------------------------
def __init_dllg_dic__():
    __path__ = currentDir+"/res/dic/dalianligong/"
    global dllg_dic_meta_list
    dllg_dic_meta_list = EmotionManager.Modules.load_dic.read_xlsx_file(__path__, "SenDic.xlsx")

#---------------------------------------
#初始化情感词极值词典
#---------------------------------------
def __init_extreme_dic__():
    __path__ = currentDir+"/res/dic/extreme_of_word/"
    global word_extreme_dic
    word_extreme_dic = EmotionManager.Modules.load_dic.load_extreme_dic(__path__, "extreme.txt")
...
def __init_dic__(dic_kind):
    global ext_dic
    jieba.load_userdict(currentDir+'/res/dic/create_by_huzehao/jieba_dic.txt')
    __path__ = currentDir+"/res/dic/zhiwang/"
    ext_dic = EmotionManager.Modules.load_dic.load_ext_dic(__path__, "extent_Lv_")  # 初始化程度副词词典
    __init__no_word_list()
    if dic_kind == 1:               #知网
        __init_zhiwang_dic__()
    elif dic_kind == 2:             #大连理工
        __init_dllg_dic__()
    elif dic_kind == 3:             #financial
        __init_ntusd_dic__()
